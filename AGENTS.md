...
- Open provider-diagnostics parity item (third accumulated change): give Gemini the same practical diagnostics UX that DeepSeek has, including equivalent log/status panels and copy/share actions. DeepSeek and Gemini must remain separate providers in the report so their requests, responses, timing, models, errors, retries/repairs, quota state and current-version totals can be audited independently.
