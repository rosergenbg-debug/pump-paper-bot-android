...
- Gemini experiment: a separate virtual account that mirrors Gemini's executed BUY entries, then tests a market-evidence exit without changing the control Gemini.
...
- Gemini experiment copies Gemini's executed BUY at the same price. Its exit evaluates buyer flow, spot/futures CVD, BTC/SOL, open interest, order book, direction and a pullback scaled to current PUMP volatility. A moderate reversal needs two monitor cycles; a strong multi-group reversal may exit immediately. A 5% loss is only an emergency backstop.
