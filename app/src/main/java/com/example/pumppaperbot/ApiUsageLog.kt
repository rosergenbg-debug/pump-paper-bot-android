...
object DeepSeekDiagnostics {
    fun report(context: Context, now: Long = System.currentTimeMillis()): String {
        ...
        return buildString {
            ...
            appendLine("модель=${primary.model} предложение=${primary.proposedAction} итог=${primary.action} исполнение=${primary.executionStatus} направление=${primary.direction} уверенность=${primary.confidence} готовностьВхода=${primary.entryReadiness}/10 опасность=${primary.danger}")
            appendLine("проверкаСделки=${primary.verificationSummary.take(500)} дыханиеВхода=${primary.lastBreathingScore}/100")
            ...
            appendLine("сценарийКороткий=${primary.shortScenario.take(500)}")
            appendLine("сценарийДлинный=${primary.longScenario.take(500)}")
            appendLine("отменаСценария=${primary.invalidation.take(500)}")
            appendLine("неопределённость=${primary.uncertainty.take(500)}")
            ...
            appendLine("экосистемаPumpFunСвежая=${ecosystem.fresh(now)} качество=${ecosystem.dataQuality}/100 оценка=${ecosystem.score ?: "нет"}/100 миграции1ч=${ecosystem.migrations1h} объём24чUSD=${ecosystem.dexVolume24hUsd ?: "нет"} доход24чUSD=${ecosystem.revenue24hUsd ?: "нет"} выкуп24чUSD=${ecosystem.buyback24hUsd ?: "нет"} сожженоPUMP24ч=${ecosystem.burnedPump24h ?: "нет"}")
            appendLine("памятьDeepSeek=${memory.russianSummary()} переданоВРешения=${memory.suppliedToDecisions}")
            appendLine()
