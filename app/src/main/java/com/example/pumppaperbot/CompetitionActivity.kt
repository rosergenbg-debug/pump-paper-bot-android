...
class CompetitionActivity : AppCompatActivity() {
    ...
    private fun render() {
        ...
        archiveStatus.text = archive.compactText() +
            "\nV6.7: полный runtime 6.5 сохранён • X AUTO имеют свежие счета • HUMAN/СЕРЖ/APP не обнулены • журнал ${ledger.trades}/${ledger.decisions}"
        ...
        val economy = V670EconomyStore.state(this)
        setChart(0, CompetitionDataset(
            "AUTO X ECONOMY • TP +2,5 / STOP -1,2 / 120m / max2",
            summary(economy.value(t32Mark), (economy.value(t32Mark) / 1000.0 - 1.0) * 100.0, economy.inPosition) + "\n${economy.reason.take(180)}",
            candles,
            economy.trades.map { CompetitionMarker(it.time, it.action, it.price, it.pnlEur) },
            T32CostPolicyV670.FEE_RATE
        ))

        val select = V670X52SelectStore.state(this)
        setChart(1, CompetitionDataset(
            "AUTO X52 SELECT • SOL/BTC L6 ≥ +0,40 • VWAP / STOP -1,2 / 90m",
            summary(select.value(t32Mark), (select.value(t32Mark) / 1000.0 - 1.0) * 100.0, select.inPosition) + "\n${select.reason.take(180)}",
            candles,
            select.trades.map { CompetitionMarker(it.time, it.action, it.price, it.pnlEur) },
            T32CostPolicyV670.FEE_RATE
        ))
