...
class GeminiExitExperimentActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        root.addView(label(
            "Тот же вход, что у рабочего Gemini. Выход — по подтверждённому ослаблению рынка.",
            14,
            "#C9D1D9",
            false
        ), params(8))
    ...
    private fun render() {
        ...
        account.text = String.format(
            ...
            if (p.inPosition) "В PUMP • вход €%.8f" else "В ЕВРО • ждём вход Gemini"
        )
        account.setTextColor(Color.parseColor(if (p.profitPercent(price) >= 0.0) "#3FB950" else "#FF7B72"))
        evidence.text = buildString {
            append("ПОСЛЕДНЯЯ ПРОВЕРКА: ${state.lastSignal}\n")
            append("Опасность ${state.lastScore}/11 • независимых групп ${state.lastGroups}/7")
            if (state.dangerStreak > 0) append(" • подтверждение ${state.dangerStreak}/2")
            if (state.adaptivePullbackPercent > 0.0) {
