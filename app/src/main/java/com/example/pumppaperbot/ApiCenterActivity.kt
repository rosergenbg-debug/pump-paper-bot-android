    ...
    private fun renderStatus() {
        if (provider == DEEPSEEK) {
            ...
            liveStatus.text = buildString {
                ...
                append("\nОсновной рынок: предложение ${state.proposedAction} • итог ${state.action} • направление ${signed(state.direction)}/100 • уверенность ${state.confidence}% • вход ${actionLevel.level}/10 • опасность ${state.danger}/10")
                append("\nИсполнение: ${state.executionStatus} • проверка: ${state.verificationSummary}")
                append("\n${state.summary}")
    ...
    private fun renderUsage() {
        ...
        usage.text = buildString {
            ...
            if (provider == DEEPSEEK) {
                ...
                append("\nV5.35 экономит запросы: основной обзор обычно раз в час, до 24 плановых вызовов/сутки; предвходный контролёр — до 6.")
