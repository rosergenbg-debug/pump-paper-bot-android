...
object HumanFactorStore {
    ...
    fun sync(context: Context, now: Long = System.currentTimeMillis()): HumanFactorState {
        // The two autonomous fixed-profit branches share the exact same market observation and
        // remain independent portfolios. Existing PumpSignalService already calls HumanFactor once
        // per full cycle, so this keeps all four T32 experiments synchronized without a new scheduler.
        T32ProfitVariantsV650.syncAll(context, now)
