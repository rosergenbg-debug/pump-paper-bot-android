...
## 2026-08-30 / V6.9.1 — Возврат полного интерфейса V6.8.1

**Root cause**
V6.9 ошибочно трактовала сокращение верхнего текста как разрешение скрывать существующие элементы. Динамический `ПОДРОБНОСТИ` при каждом запуске ставил `GONE` кнопкам и информационным блокам.

**Change**
Механизм скрытия удалён полностью; порядок и видимость интерфейса V6.8.1 восстановлены. Верхние карточки только уплотнены без потери данных. BTC 2/6/24ч размещены вертикально слева внутри графика. ±1.5 presentation guides заменены адаптивной целой сеткой ±1/±2/±3/±4. Кнопка сети `БОТЫ` визуально выделена. Экспорт V6.9, денежный поток, счета, stores и торговая логика сохранены.

**Verification**
`RangeGuidePolicyTest` фиксирует точные уровни 1–4%; source scan запрещает возврат V6.9 details-hiding identifiers; полный Android CI обязателен перед выдачей.
