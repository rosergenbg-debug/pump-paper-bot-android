...
Снимок фактической архитектуры `main` на 2026-08-26, V5.36. Это описание существующего кода, а не целевая архитектура.
...
- `MicroImpulseStream` запускает быстрые локальные shock-check без обязательного AI-запроса.
- Главный цикл защищён от параллельного повторного запуска.
...
Обязательный `MarketSyncClient.sync()` находится раньше failure-isolated optional stages: его полная ошибка может оборвать текущий цикл.
...
- `LiquidityReleaseShadow` — V5.32 причинный seller-exhaustion/liquidity-release observer. Он сохраняет evidence, но не имеет торгового authority.
...
Все четыре используют `PumpProfitEngineV526` для текущего PM entry/position logic, но имеют отдельные portfolios/state/cooldowns.
...
2. Сформировать multi-horizon flow/breath snapshot.
3. `AdaptiveBreathEntryPolicy` считает score и hard veto.
4. `PumpProfitEngineV526` поддерживает candidate/hysteresis/confirmation и price acceptance.
5. Если локальный BUY готов, `DeepSeekEntryCoach.review()` может подтвердить/отклонить по разрешённому soft contract либо использовать строго ограниченный fallback.
6. При разрешении paper BUY записывается по ask с fee; portfolio/state сохраняются отдельно.
7. Решение и причина идут в local store и unified log.
...
`CompetitionActivity` сейчас отображает **8 вертикальных графиков** с синхронизированным horizontal offset: четыре Pump Machine, Fusion, DeepSigX, APP, SERGE. Нажатие графика открывает детальный диалог со сделками.
...
V5.0 начинался как research baseline с APP/DeepSig/DeepSigX и общим причинным replay-подходом. V5.21–V5.36 поверх него вырос отдельный fast Pump Machine/Fusion контур. Оба слоя живы одновременно. Это не обязательно ошибка, но граница между «research baseline» и «активным PM execution experiment» сейчас не выражена одним явным интерфейсом.

`NEEDS_VERIFICATION`: текущие Pump Machine decision paths имеют ли полноценный replay/walk-forward harness, который воспроизводит тот же 15-second flow/book/DeepSeek-coach contract. Для исходного `ResearchDecisionEngine` такой подход документирован; для полного V5.36 PM-контура эквивалентность пока не установлена.

## B. Имена больше не отражают смысл

- UI `Pump Machine 1` фактически использует `PumpMachine2Store`/PUMP_2.
- UI `Pump Machine 2` использует класс `PumpMachineStore`, исторически описанный как +3% Pump Machine.
- DeepSigX живёт в Gemini-named storage/classes.
- Application class называется `V513Application` при текущей версии V5.36.
...
`NEEDS_VERIFICATION`: покрыты ли тестами все реальные upgrade-paths старых совместимых установок до V5.36.

## E. Patch-oriented история разработки

Каталог `tools/` содержит последовательные `apply_v519_*` … `apply_v526_*` scripts. Они полезны как исторический след, но показывают, что значимая часть развития шла наслоением version patches. Нельзя автоматически продолжать этот паттерн при следующей серии ошибок.

## F. Частично устаревшие комментарии/документация

`ResearchModePolicy` всё ещё говорит о «трёх автономных системах», тогда как текущий service/UI управляет более широким набором независимых paper accounts. Старые version-specific README-разделы также описывают правила своей версии и не являются текущей спецификацией.

## G. Release-state drift

Исходники `main` и CI находятся на V5.36, совместимый V5.36 APK был отдельно проверен и подписан, но GitHub `latest release` остаётся V4.9 и tag `v5.36` отсутствует. Значит «current source», «verified compatible APK» и «published Release» сейчас три разных понятия.
