...
## 2026-08-28 — BTC/SOL не получают право подтверждать дно PUMP

**Decision**
Оставить BTC/SOL режимным контекстом. Не подключать исследованные cross-market turn-фильтры к production BUY и не считать совместный рост BTC/SOL доказанным подтверждением дна PUMP.

**Reason**
На 14-дневном причинном replay с временным train/test-разделением заметна связь в ту же минуту, но отсутствует устойчивое опережение на 1–10 минут. Лучший простой контрольный вариант изменил PM-like win rate лишь с 31,3% до 32,3%; недельный результат 41,2% не воспроизвёлся.

**Consequences**
Совместное сильное падение BTC/SOL допустимо логировать как предупреждение о режиме, но пока не как новый hard veto. Синтетические сценарии подтверждают смысл классификации, а не прибыльность. Текущие стратегии, версия и сборка не меняются.
# 2026-08-28 — Пять контекстных гипотез не получают торговую власть

**Decision**
Не подключать к production-счетам regime-adaptive entry, минутный liquidity sweep, 60m VWAP mean-reversion, sell exhaustion и комбинацию 4H node + relative strength + SELL decay + EU session в протестированных фиксированных формулах.

**Reason**
На причинном 120-дневном replay ни одна формула не дала положительный средний NET или PF >1 ни в early, ни в control. Лучший control win rate 32,65%, лучший Avg NET −0,419%, лучший PF 0,399. Synthetic 10/10 подтверждает только правильность ветвления.

**Consequences**
Production/version не меняются. Пороги на просмотренном окне не оптимизировать. Если возвращаться к sweep/absorption, сначала нужен trade-level tape/order-book replay.

---
# 2026-08-28 — Cost-aware VWAP и session gap-fill отклонены

**Decision**
Не подключать cost-aware VWAP и EU session gap-fill к production paper-счетам.

**Reason**
На отдельном ранее не использованном 120-дневном окне обе формулы дали 8,33% win rate на control, отрицательный средний NET и PF значительно ниже 1. Прежний VWAP control WR 32,65% не воспроизведён.

**Consequences**
Не подбирать новые VWAP/session пороги на просмотренных окнах. Следующая принципиально новая проверка должна использовать trade tape/L2 для OFI, replenishment/absorption или trade-size shift.

---
# 2026-08-28 — Четыре фиксированные OHLCV-гипотезы не продвигаются

**Decision**
Не подключать Z-Score VWAP, volatility squeeze breakout, strict lead-lag и session-open squeeze к production paper-счетам.

**Reason**
На третьем независимом блоке лучший control WR составил 14,29%; все измеренные Avg NET отрицательны и PF ниже 1. Early squeeze 50% на 24 fills не воспроизвёлся. Strict lead-lag дал 0 сигналов.

**Consequences**
Не ослаблять пороги на просмотренном блоке. CVD/L2 идеи требуют нового trade-tape/order-book dataset.

---
# 2026-08-28 — Двойной VWAP 32,65 эксперимент с Human Factor

**Decision**
Добавить два независимых paper-счёта на исходной VWAP-формуле: AUTO самостоятельно исполняет виртуальные сделки без звуков; HUMAN FACTOR требует подтверждения владельца перед каждым входом.

**Reason**
Исходный VWAP дал лучший наблюдавшийся control win rate 32,65%, хотя остался NET-убыточным. Параллельные счета позволяют измерить, улучшает ли человеческий контекст результат относительно чистого алгоритма, не смешивая portfolios.

**Consequences**
Human Factor предупреждает на 90/100 и держит подтверждение при 98–100/100 до решения пользователя или распада сигнала. AUTO не уведомляет. Реальные ордера отсутствуют. Другие стратегии остаются активными исследователями, но их виртуальные trade-звонки отключены; ручные safety/exit alerts сохранены.

---
# 2026-08-28 — V6.4 показывает только четыре основных счёта

**Decision**
Сократить пользовательский экран сети до T32, Human Factor, СЕРЖ и APP в этом порядке. Старые виртуальные стратегии не удалять и не останавливать, а только скрыть из основного интерфейса.

**Reason**
Владелец выбрал T32 главным ориентиром и хочет сравнивать его только с ручным Human Factor, своим счётом и APP без визуального шума старых экспериментов.

**Consequences**
История старых счетов сохраняется. DeepSeek остаётся сервисом анализа и диагностики, но не участником сети; его компактный верхний статус обязан показывать подключение, ошибки и дневной расход.

---


## 2026-09-12 — Preserve Windows >=80% evidence in X

Владелец явно поручил перенести стратегии от +80% с максимальным контекстом восстановления. Порог применяется к хотя бы одному сохранённому общему development/regime/preview NET, а не к win rate и не обязательно ко всем рынкам. Сохраняются также связанные меньшие результаты, точные строки из резервной копии и рабочие снимки. Архивные результаты отделены от подтверждённых торговых checkpoints; недостающие trade ledgers/старые исходные сборки не выдумываются. Подробности и проверяемый manifest: `X/WINDOWS_80PLUS_2026-09-10/`.
